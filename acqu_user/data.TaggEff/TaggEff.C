...
Char_t clockhist[50] ="SumScalers190to191";
Int_t clockhistno =1;
...
cout << "CAM Test 1" <<endl;
  //Read in the files to be analysed
  scan_files(runlist);

cout << "CAM Test 2" <<endl;
...
cout << "CAM Test 3" <<endl;
  //Define the histograms to be filled
  for (Int_t i=0;i<nsets;i++){

    sprintf(dirname,"Run_%i",i);
    dir[i] = rootout->mkdir(dirname);
    dir[i]->cd();
...
void scan_files(Char_t* runlist)
{
  ...
  if(list.is_open()){
	...
	if(!strcmp(line,"Prompt-Correction:"))
	  list>>accsub;

	if(!strcmp(line,"Clock-Histogram:"))
	  list>>clockhist;

	if(!strcmp(line,"Clock-Histogram-Bin:"))
	  list>>clockhistno;
...
void define_histos(Int_t setno, Int_t useEg)
{
  ...
  for(Int_t i=2;i<files;i++)
	...
	sprintf(lname,"Perpendicular");
      ...
      time[setno][i]=new TH1F(hname,htitle,1000,-250,250);
...
/*-------------------------------------------------------------------------*/
//Determine the tagging efficiency etc.
void taggeff_ana(Int_t setno, Int_t useEg, Int_t usesub)
{
  TH1F *fscalers;
  TH1F *prompts;
  TH1F *accidentals;
  ...
  for(Int_t i=0;i<files;i++)
	...
	noBG=1;
      ...
      clocks =(TH1F*)file->Get(clockhist);
      if(!clocks){
	cout<<"Clocks histogram not defined for "<<filename[setno][i]<<"!"<<endl;
	cl_ok=0;
	...
	prompts = (TH1F*)file->Get("FPD_Hits");
      }
	...
	if(cl_ok)
	  t_hallbg+=clocks->GetBinContent(clockhistno);
      }

      /* Radiator file section*/
      if(i>1){
	//	cout<<filename[setno][i]<<endl;
